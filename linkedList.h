...
void quick_sort(list_t*, int, int);
...
int partition(list_t* slice, int min, int max){
	int pivot;
    get_list(slice, &pivot, max);
	int i = min - 1;

	for(int j = min; j < max; j++){
        int sliceJ;
        get_list(slice, &sliceJ, j);

		if(sliceJ < pivot){
			i++;
			int tmp2;
            get_list(slice, &tmp2, i);
            replace_in_list(slice, sliceJ, i);
            replace_in_list(slice, tmp2, j);
		}
	}
	i++;
	int tmp;
    get_list(slice, &tmp, i);
    replace_in_list(slice, slice->tail->val, i);
    slice->tail->val = tmp;
	return i;
}

void quick_sort(list_t* slice, int min, int max){
	if(min <= max){
		int pi = partition(slice, min, max);

		quick_sort(slice, min, pi-1);
		quick_sort(slice, pi+1, max);
	}
